...
async def analyze_sentiment() -> Dict[str, Union[float, str]]:
    ...
    try:
        ...
        title_list = await youtube.get_title()
        ...
        if '6 swear words or less' in vulgarity_check or "Lyrics go to NLP model" in vulgarity_check:
            ...
            sentiment_result = await analyze_sentiment_async(lyrics=cleaned_lyrics, title=title_list[1])
            ...
            #logging.info(vulgarity_check)
            logging.info("Too many swear words")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(koyeb_url, json=payload) as response:
                    if response.status == 200:
                        ...
                        logger.info(f"{payload}, {koyeb_url}")
                        if result != "null":
                            return result
                    else:
